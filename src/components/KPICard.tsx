...
const KPICards = ({ data, mode = "live" }) => {
  if (mode === "live") {
    ...
    const avgAccelRms =
      data.reduce((sum, item) => sum + (item.accelRms || 0), 0) / data.length;
    const maxAccelMax = Math.max(...data.map((item) => item.accelMax || 0));
    const avgVelocityRms =
      data.reduce((sum, item) => sum + (item.velocityRms || 0), 0) /
      data.length;
    const avgCrest =
      data.reduce((sum, item) => sum + (item.crest || 0), 0) / data.length;
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <Kpi
          title="Avg Peak-to-Peak"
          value={
            data.reduce((sum, item) => sum + (item.accelPP || 0), 0) /
            data.length
          }
          unit="mg"
        />
